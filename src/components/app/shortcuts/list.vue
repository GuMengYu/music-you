<template>
  <div class="shortcuts" :style="{'--column-count': columnCount, '--grid-gap': gridGap}">
    <Shortcut :data="state.myFav" v-if="logged" :flag="{color: 'primary', label: 'A'}" />
    <Shortcut :data="state.daily" type="daily" :flag="{color: 'secondary', label: 'B'}" />
    <Shortcut :data="state.radar" :flag="{color: 'tertiary', label: 'C'}" />
    <Shortcut :data="state.myFav" v-if="logged" :flag="{color: 'error', label: 'D'}"  />
...
const state = reactive({
  radar: {
    ...
    subTitle: '',
    picUrl:
        'https://p2.music.126.net/0VRN6GBaPibXxfKz2UbzdA==/109951165959686617.jpg',
  },
  daily: {
