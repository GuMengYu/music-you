<template>
  <v-dialog v-model="showLogin" persistent>
    <v-card outlined color="surface" class="login-container py-6" rounded="xl">
      ...
      >
      <div class="form-area mx-6 py-9" v-if="state.loginType === LOGIN_TYPE.ACCOUNT">
        <v-text-field
          outlined
          filled
          dense
      ...
      <div
        ...
        v-if="state.loginType === LOGIN_TYPE.QRCODE"
      >
        <v-card
          outlined
        ...
        >
          ...
          >
            <v-avatar size="100"
              ><v-img :src="state.qrHeadImage" lazy-src="src/assets/placeholder.png"
            /></v-avatar>
          </v-progress-circular>
          <v-btn
            color="primary"
            ...
            small
            @click="genCode"
            v-if="state.qrState === QR_STATUS.EXPIRED"
...
import { mdiLogin, mdiLock, mdiPacMan, mdiReload } from '@mdi/js';
