...
import { toggleTrackLike, getPlayList } from '@/pages/local/api/local'
import { queryLikedTracks } from '@/pages/local/hooks/useQueryTrack'
...
export const useLocalStore = create(persist<RootState & SettingAction>((set, get) => {
  return {
    ...
    async refreshLikes() {
      const res = await queryLikedTracks()
      set({
