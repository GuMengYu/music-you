...
export function LocalLikeToggle({
  id,
  size,
  color,
}: {
...
export default function LikeToggleButton({
  ...
  const isLocal = track?.source.fromType === 'local'
  if (isLocal)
    return <LocalLikeToggle id={track.id} color={color} size={size}></LocalLikeToggle>
  else
    return <LikeToggle id={track.id} color={color} size={size}></LikeToggle>
